...
```
> 在现代浏览器中，type="text/javascript"不写也是可以的，默认就是该值。

(3)可以将代码编写到外部的JavaScript文件中，然后通过标签将其引入：

```html
<script type="text/javascript" src="文件路径">
</script>
```
> script 标签一旦用于引入外部文件了，就不能在编写代码了，即使编写了浏览器也会忽略，如果需要可以创建新的script标签用于编写内部代码。
...
- JavaScript严格区分大小写
- JavaScript中每条语句以分号(;)结尾
- JavaScript中会自动忽略多个空格和换行

> JavaScript语句结尾不写分号浏览器会自动添加，但会消耗系统资源，有时候，浏览器会加错分号。
...
> 字母包括**ASCII字符**与**Unicode字符**(如中文字符)。
...
>  JavaScript中一共分成六种数据类型：5个基本数据类型和Object。

### String字符串

(1)JavaScript中的字符串需要使用引号引起来：
...
### Number数值
(1)JavaScript中所有的整数和浮点数都是Number类型
...
- 二进制：0b 或0B开头表示二进制，但是不是所有的浏览器都支持二进制
...
### 4.Null空值
空值专门用来表示为空的**对象**，Null类型的值只有一个：null
...
## 类型转换
 ...
 类型转换就是指将其他的数据类型，转换为String、Number或Boolean
...
> 这个方法**不适用于null和undefined**，因为这两个类型的数据中没有方法，所以调用toString()时会报错。
...
console.log(typeof a); // string
```
> (1)对于Number、Boolean、String都会调用他们的toString()方法来将其转换为字符串。
>
> (2)对于null，直接转换为字符串"null"。
>
> (3)对于undefined，直接转换为字符串"undefined"。
...
(2)调用parseInt()或parseFloat()
 ...
 ```javascript
var a = parseInt("123.456px");
console.log(a); // 123
console.log(typeof a); // number
 ```
> 如果对非String使用parseInt()或parseFloat()，它会**先将其转换为String**然后在操作parseInt()，将**一个字符串中的有效的整数位**提取出来，并转换为Number。

- parseInt(string, radix)
    - string：必需，要解析的字符串。
    - radix：可选，整数的进制，代表要使用的数字系统的数字(从2到36)，默认是10。
...
> 除了**加法**以外，对非Number类型的值进行运算时，都会先转换为Number然后在做运算。
>
...
(1)正号(+)：不会对值产生任何影响，但是可以将一个非数字转换为数字
...
var a = true;
console.log(-a); // -1
console.log(typeof -a); // numbervar a = 10;
```

(3)自增(++)：自增可以使变量在原值的基础上自增1
...
优先级：
 和数学中一样，JavaScript中的运算符也是具有优先级的，
    比如 先乘除 后加减 先与 后或
...
**函数**(function)：也是一个对象，也具有普通对象的功能。

(1)一个简单的函数应用：

```javascript
// 函数声明
function test(a) { // test是函数名，a是形参
    console.log(a);
}
// 函数的调用
test(10);
...
> (1)示例中的a是形参，可以包含任意个的形参(0个或者多个)；
>
> (2)多个形参之间使用(,)分隔；
>
> (3)JavaScript是一个**弱类型**的语言，所以函数声明的时候，形参并不写类型；
>
> (4)函数调用的时候传入的数据就是实参，由于形参没有数据类型，所以实参可以传递任意类型的值；
>
> - 如果实参的数量大于形参，多余实参将不会赋值
> - 如果实参的数量小于形参，则没有对应实参的形参将会赋值undefined
>
> (5)JavaScript不允许函数重载，函数的重载会直接覆盖掉上一次的定义。

(2)使用typeof检查一个函数时会返回function

```javascript
console.log(typeof test) // function
```

> (1)使用typeof检查的时候，**直接写函数名**就是可以；
>
> (2)如果加上实参，则表示函数调用，检查的是函数的返回值类型，没有返回值时，就是undefined。

(3)函数表达式：创建一个匿名函数，将这个匿名函数赋值给变量
...
// 匿名函数的调用
test(10); // 如果函数定义的时候有形参，则就需要传入实参
console.log(typeof test); // function
```

> **立即执行函数**：函数定义完，立即被调用，这种函数叫做立即执行函数，立即执行函数往往**只会执行一次**。
...
// 函数的定义
function sum(a, b) {
    // JavaScript是弱类型语言，不检查参数类型，我们可以自己手动检查
...
> (1)如果return后不跟值，表示结束函数；
>
> (2)如果不写return则函数默认返回undefined，就是表示没有返回值。

(5)隐形参数：arguments

- 函数中不需要定义，但却可以直接用来获取**所有的参数**的变量叫隐形参数。

- 隐形参数特别像Java的可变参数一样，**操作类似数组**。
- 不一样的点在于Java的可变参数是收集**剩下的参数**，而JavaScript是**所有的参数**。

```javascript
function fun() {
    for (var i = 0; i < arguments.length; i++) {
        console.log(arguments[i]); // 将传入的参数逐一打印
    }
}
fun(10, 40, 50); // 此时，arguments.length = 3
```
...
console.log(sum(10, 60, "50")); // 70
```

> (1)arguments[0]一定是实参的第1个，无论函数定义的时候有多少个形参，因为arguments获取的是**所有参数**；
>
> (2)当实参个数为0的时候，arguments[0]=undefined，下面示例中的代码依旧是正确的，因为此时a也是undefined。

```javascript
function sumFun(a, b) {
    if (arguments[0] === a) { // 此条件永远为true
        console.log("条件成立");
    }
}
```
...
(1)方法(method)：可以将一个函数设置为一个对象的属性，当一个对象的属性是一个函数时，我们称这个函数是该对象的方法。
...
// 创建一个对象
var obj = new Object();
// 将一个函数赋值给该对象的属性，该属性称为这个对象的方法
obj.sum = sumNumber; // 不加形参
// 方法的调用
console.log(obj.sum(10, 25)); // 35
```
...
(1)作用域简单来说就是一个变量的作用范围。

(2)在JavaScript中作用域分成两种：
...
- 全局作用域在**打开页面时创建，在页面关闭时销毁**。
- 全局作用域中有一个全局对象window，window对象由浏览器提供，可以在页面中直接使用，它代表的是整个浏览器的窗口。
...
> **变量的声明提前**：
>
> (1)在全局作用域中，使用**var**关键字声明的变量会在所有的代码执行之前被**声明**，但是**不会赋值**。
>
> (2)在函数作用域中，也具有该特性，使用**var**关键字声明的变量会在函数所有的代码执行前被声明，如果没有使用**var**关键字声明变量，则变量会变成全局变量。

> **函数的声明提前**
>
> (1)在全局作用域中，使用**函数声明**创建的函数，会在所有的代码执行之前被创建。
>
> (2)使用**函数表达式**创建的函数没有该特性，声明不会被提前。
...
日期的对象，在JavaScript中通过Date对象来表示一个时间
创建对象
 创建一个当前的时间对象
...
文档的加载
 ...
 如果将JavaScript代码编写到页面的上边，当代码执行时，页面中的DOM对象还没有加载，
    此时将会无法正常获取到DOM对象，导致DOM操作失败。
 解决方式一：
     可以将JavaScript代码编写到body的下边
 ...
 **通过style修改和读取的样式都是内联样式**，由于内联样式的优先级比较高，
    所以我们通过JavaScript来修改的样式，往往会立即生效，
...
JavaScript中的对象只有JavaScript自己认识，其他的语言都不认识
**JSON就是一个特殊格式的字符串**，这个字符串可以被任意的语言所识别，
并且可以转换为任意语言中的对象，JSON在开发中主要用来数据的交互
 JSON和JavaScript对象的格式一样，只不过**JSON字符串中的属性名必须加双引号**
    其他的和JavaScript语法一致
JSON分类：
    1.对象 {}
    2.数组 []
...
JSON.parse()
 可以将以JSON字符串转换为js对象
 它需要一个JSON字符串作为参数，会将该字符串转换为JavaScript对象并返回
